class GoogleBikeShopService
  ...
  def self.shop_query(location)
    ...
    response = Faraday.get("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=#{lat_long[:lat]},#{lat_long[:lng]}&radius=10000&key=#{ENV['GOOGLE_KEY']}&keyword=bikerepair&types=bicycle_store&rankBy=distance")
